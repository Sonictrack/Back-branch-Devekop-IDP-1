...
export default {
  ...
  mounted () {
    if (
      !this.isLoggedIn ||
      this.getCurrentUser.type !== UserType.partner
    ) {
      this.redirect()
    }
  },
  methods: {
    ...
    async verify () { 
      ...
      if (json.clientNumber) { 
        let notifContent= `Votre ${json.type} numéro #${json.numero}# a fait l'objet d'un contrôle soumis à votre décision par (${this.getCurrentUser.companyName}) .`
    ...
    async checkHandshake (idDocument) {
      ...
      if (json.request.status === 'confirm') {
        ...
        let notifContent= `Vous avez donné votre consentement à une entrée en relation avec (${this.getCurrentUser.companyName}) sur présentation du/la ${this.documentData.type} numéro #${this.documentData.numero}#.`
      ...
      if (json.request.status === 'reject') {
        ...
        this.rejected = true
        // send notif accept request
        let notifContent= `Vous avez refusé l'entrée en relation avec (${this.getCurrentUser.companyName}) sur présentation de/la ${this.documentData.type} numéro #${this.documentData.numero}#.`
    ...
    async handleDocumentRequest (json) {
      ...
      this.currentTimer = setTimeout(() => {
        ...
        let notifContent= `Vous avez reçu une demande de mise en relation de (${this.getCurrentUser.companyName}) sur présentation de/la ${this.documentData.type} numéro #${this.documentData.numero}# à laquelle vous n'avez pas répondu.` 
